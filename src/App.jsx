...
function NotificationCenter({items,setItems,close,go,onRead,onReadAll,onDelete,loading,error}){
 ...
 const open=async n=>{if(!n.read)await onRead(n.id);close();if(n.page)go(n.page)};
 const icon=n=>n.type==='payment'?<ReceiptText/>:n.type==='offer'?<FileText/>:n.type==='job'||n.type==='job_status'?<BriefcaseBusiness/>:n.type==='maintenance'||n.type==='schedule'?<CalendarDays/>:<Package/>;
 return <div className="nav-pop notifications-pop" ref={ref}>
  ...
  <div className="notification-tools"><div className="mini-tabs"><button className={filter==='Sve'?'active':''} onClick={()=>setFilter('Sve')}>Sve</button><button className={filter==='Nepročitane'?'active':''} onClick={()=>setFilter('Nepročitane')}>Nepročitane</button></div><button disabled={!items.some(n=>!n.read)} onClick={onReadAll}><Check/> Označi sve pročitano</button></div>
  <div className="notification-list">{loading?<div className="notification-empty"><Bell/><strong>Učitavanje...</strong></div>:error?<div className="notification-empty"><AlertTriangle/><strong>Greška</strong><span>{error}</span></div>:visible.length?visible.map(n=><article key={n.id} className={!n.read?'unread':''} onClick={()=>open(n)}><div className={`notification-icon ${n.type}`}>{icon(n)}</div><div><strong>{n.title}</strong><p>{n.text}</p><span>{relTime(n.created_at)}</span></div>{!n.read&&<i/>}<button title="Ukloni" onClick={e=>{e.stopPropagation();onDelete(n.id)}}><X/></button></article>):<div className="notification-empty"><Bell/><strong>Nema obavijesti</strong><span>Nove obavijesti pojavit će se nakon stvarnih događaja u aplikaciji.</span></div>}</div>
...
function AppShell(){
 ...
 const [navPopup,setNavPopup]=useState(null); const [quickCreate,setQuickCreate]=useState(null); const [profileModal,setProfileModal]=useState(false); const [searchOpen,setSearchOpen]=useState(false); const [notifications,setNotifications]=useState([]),[notificationsLoading,setNotificationsLoading]=useState(false),[notificationsError,setNotificationsError]=useState(''); const [notice,setNotice]=useState('');
 ...
 const loadNotifications=async(silent=false)=>{if(!authed)return;if(!silent)setNotificationsLoading(true);try{const r=await fetch('/api/notifications',{credentials:'include'}),d=await r.json();if(!r.ok)throw new Error(d.error||'Obavijesti nije moguće učitati.');setNotifications(Array.isArray(d)?d:[]);setNotificationsError('')}catch(e){setNotificationsError(e.message)}finally{setNotificationsLoading(false)}};
 useEffect(()=>{if(!authed)return;loadNotifications();const timer=setInterval(()=>loadNotifications(true),60000);return()=>clearInterval(timer)},[authed]);
 const readNotification=async id=>{const r=await fetch(`/api/notifications/${id}/read`,{method:'PATCH',credentials:'include'}),d=await r.json();if(!r.ok)throw new Error(d.error);setNotifications(a=>a.map(n=>String(n.id)===String(id)?{...n,read:true,read_at:new Date().toISOString()}:n))};
 const readAllNotifications=async()=>{try{const r=await fetch('/api/notifications/read-all',{method:'POST',credentials:'include'}),d=await r.json();if(!r.ok)throw new Error(d.error);setNotifications(a=>a.map(n=>({...n,read:true,read_at:n.read_at||new Date().toISOString()})))}catch(e){notify(e.message||'Obavijesti nije moguće ažurirati.')}};
 const deleteNotification=async id=>{try{const r=await fetch(`/api/notifications/${id}`,{method:'DELETE',credentials:'include'}),d=await r.json();if(!r.ok)throw new Error(d.error);setNotifications(a=>a.filter(n=>String(n.id)!==String(id)))}catch(e){notify(e.message||'Obavijest nije moguće ukloniti.')}};
 ...
 return <div className="app">
  ...
  <main className={isOwner?'owner-main':''}><header><button className="menu-btn" onClick={()=>setMobile(true)}><Menu/></button><button className="global-search global-search-button" onClick={()=>!isOwner&&setSearchOpen(true)}><Search/><span>{isOwner?'Pretraži firme i korisnike...':'Pretraži klijente, poslove, račune...'}</span><kbd>⌘ K</kbd></button><div className="header-actions">
   ...
   <div className="header-pop-wrap"><button className={`bell ${navPopup==='notifications'?'active':''}`} onClick={()=>{const next=navPopup==='notifications'?null:'notifications';setNavPopup(next);if(next)loadNotifications()}}><Bell/>{notifications.some(n=>!n.read)&&<i></i>}</button>{navPopup==='notifications'&&<NotificationCenter items={notifications} setItems={setNotifications} close={()=>setNavPopup(null)} go={go} onRead={readNotification} onReadAll={readAllNotifications} onDelete={deleteNotification} loading={notificationsLoading} error={notificationsError}/>}</div>
