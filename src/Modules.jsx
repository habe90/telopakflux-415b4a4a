...
function Empty(){return <div className="empty-row">Nema podataka za prikaz.</div>}
function moneyNumber(value){if(typeof value==='number')return Number.isFinite(value)?value:0;const raw=String(value||'').replace(/[^0-9,.-]/g,'');if(!raw)return 0;const normalized=raw.includes(',')?raw.replace(/\./g,'').replace(',','.'):raw;const n=Number(normalized);return Number.isFinite(n)?n:0}
function money(value){return new Intl.NumberFormat('bs-BA',{style:'currency',currency:'EUR'}).format(Number(value)||0)}
function dateInCurrentMonth(value){const d=parseEUFull(value);const now=new Date();return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()}
function dateInCurrentYear(value){const d=parseEUFull(value);return d.getFullYear()===new Date().getFullYear()}

export function ClientsModule(){
 const {clients:items,setClients:setItems,jobs,invoices}=useData();
 ...
 const clientStats=name=>{const relatedJobs=jobs.filter(j=>j.client===name);const relatedInvoices=invoices.filter(i=>i.client===name);return {jobs:relatedJobs.length,value:relatedInvoices.reduce((sum,i)=>sum+moneyNumber(i.paid),0)}};
 ...
 const activeJobs=jobs.filter(x=>x.status!=='Završeno').length;
 const monthPaid=invoices.filter(x=>dateInCurrentMonth(x.issued)).reduce((sum,x)=>sum+moneyNumber(x.paid),0);
 ...
 return <><Header eyebrow="CRM I ISTORIJA POSLOVA" title="Klijenti" description="Kontakti, lokacije i kompletna istorija saradnje." button="Novi klijent" onAdd={()=>setModal({type:'form',data:{type:'Fizičko lice',status:'Aktivan'}})}/><div className="module-metrics"><Metric icon={Users} label="Ukupno klijenata" value={items.length} sub={`${items.filter(x=>dateInCurrentMonth(x.created_at)).length} novih ovaj mjesec`}/><Metric icon={BriefcaseBusiness} label="Aktivni poslovi" value={activeJobs} sub={`Kod ${new Set(jobs.filter(x=>x.status!=='Završeno').map(x=>x.client)).size} klijenata`} tone="green"/><Metric icon={Euro} label="Naplaćena saradnja" value={money(monthPaid)} sub="Ovaj mjesec" tone="orange"/></div><section className="card"><Toolbar search={search} setSearch={setSearch} placeholder="Ime, kontakt ili grad..." statusOptions={['Svi statusi','Aktivan','Novi upit','Neaktivan']} statusValue={status} setStatusValue={setStatus} onExport={exportList}/><div className="table-wrap"><table><thead><tr><th>Klijent</th><th>Kontakt osoba</th><th>Lokacija</th><th>Poslovi</th><th>Vrijednost</th><th>Status</th><th>Akcije</th></tr></thead><tbody>{filtered.map(x=>{const stats=clientStats(x.name);return <tr key={x.id} className="clickable-row" onClick={()=>setSelected({...x,...stats})}><td><div className="client-cell"><span className="avatar square">{x.name.split(' ').map(y=>y[0]).slice(0,2)}</span><div><strong>{x.name}</strong><small>{x.type}</small></div></div></td><td><strong>{x.contact}</strong><small className="table-sub">{x.email}</small></td><td>{x.city}<small className="table-sub">{x.address}</small></td><td>{stats.jobs}</td><td><strong>{money(stats.value)}</strong></td><td><Badge>{x.status}</Badge></td><td><Actions onView={()=>setSelected({...x,...stats})} onEdit={()=>setModal({type:'form',data:x})} onDelete={()=>setModal({type:'delete',data:x})}/></td></tr>})}</tbody></table>{!filtered.length&&<Empty/>}</div></section>{modal?.type==='form'&&<ClientForm data={modal.data} close={()=>setModal(null)} save={save}/>} {modal?.type==='delete'&&<ConfirmDelete name={modal.data.name} onClose={()=>setModal(null)} onConfirm={()=>del(modal.data)}/>} {selected&&<DetailDrawer title={selected.name} sub={`${selected.type} · ${selected.city}`} close={()=>setSelected(null)} actions={<><button className="secondary" onClick={()=>exportToPDF(`Klijent — ${selected.name}`,`${selected.type} · ${selected.city}`,['Podatak','Vrijednost'],[['Kontakt osoba',selected.contact],['Email',selected.email],['Telefon',selected.phone],['Adresa',`${selected.address}, ${selected.city}`],['Broj poslova',selected.jobs],['Ukupna vrijednost',money(selected.value)],['Status',selected.status]])}><Download/> PDF</button><button className="secondary" onClick={()=>setModal({type:'form',data:selected})}><Edit3/> Uredi</button><button className="primary"><Plus/> Novi posao</button></>}><div className="drawer-profile"><span className="avatar profile-avatar">{selected.name.split(' ').map(y=>y[0]).slice(0,2)}</span><div><h3>{selected.contact}</h3><Badge>{selected.status}</Badge></div></div><InfoGrid items={[["Email",selected.email],["Telefon",selected.phone],["Adresa",`${selected.address}, ${selected.city}`],["Broj poslova",selected.jobs],["Ukupna vrijednost",money(selected.value)]]}/><div className="drawer-section"><h4>Interna bilješka</h4><p>{selected.note||'Nema unesenih bilješki.'}</p></div><div className="drawer-section"><h4>Historija saradnje</h4>{jobs.filter(j=>j.client===selected.name).length||invoices.filter(i=>i.client===selected.name).length?<>{jobs.filter(j=>j.client===selected.name).slice(-3).reverse().map(j=><div className="activity-line" key={`j-${j.id}`}><Wrench/><div><strong>{j.service||'Posao'}</strong><span>{j.date||'Bez datuma'} · {j.status}</span></div></div>)}{invoices.filter(i=>i.client===selected.name).slice(-3).reverse().map(i=><div className="activity-line" key={`i-${i.id}`}><ReceiptText/><div><strong>Račun {i.no}</strong><span>{i.issued||'Bez datuma'} · {i.status}</span></div></div>)}</>:<p>Nema evidentiranih poslova ili računa za ovog klijenta.</p>}</div></DetailDrawer>}</>}
...
function DocumentModule({kind}){
 ...
 const sentOfferCount=isOffer?items.filter(x=>['Poslata','Prihvaćena','Istekla'].includes(x.status)).length:0;
...
export function StockModule(){
 ...
 const del=x=>{setItems(a=>a.filter(i=>i.id!==x.id));setModal(null)};
 const stockValue=items.reduce((sum,x)=>sum+(Number(x.stock)||0)*moneyNumber(x.buy),0);
 const categories=new Set(items.map(x=>x.category).filter(Boolean)).size;
...
export function MaintenanceModule(){
 ...
 const upcoming30=items.filter(x=>{const d=parseEUFull(x.next),diff=(d-new Date())/86400000;return diff>=0&&diff<=30}).length;
 const renewed=items.filter(x=>dateInCurrentYear(x.next)&&/obnov|završ/i.test(x.status||'')).length;
...
export function ReportsModule(){
 const {jobs,offers,invoices}=useData();
 const workerNames=['Svi radnici',...Array.from(new Set(jobs.map(j=>j.worker).filter(Boolean)))];
 const [worker,setWorker]=useState('Svi radnici');
 const filteredJobs=worker==='Svi radnici'?jobs:jobs.filter(j=>j.worker===worker);
 const completed=filteredJobs.filter(j=>j.status==='Završeno');
 const revenue=invoices.reduce((sum,i)=>sum+moneyNumber(i.paid),0);
 const sentOffers=offers.filter(o=>['Poslata','Prihvaćena','Istekla'].includes(o.status));
 const accepted=offers.filter(o=>o.status==='Prihvaćena').length;
 const rate=sentOffers.length?Math.round(accepted/sentOffers.length*100):0;
 const workers=workerNames.slice(1).map(name=>{const all=jobs.filter(j=>j.worker===name),done=all.filter(j=>j.status==='Završeno').length;return [name,done,all.length?Math.round(done/all.length*100):0]});
 const services=Object.values(completed.reduce((acc,j)=>{const key=j.service||'Bez naziva';if(!acc[key])acc[key]={name:key,count:0,revenue:0};acc[key].count++;acc[key].revenue+=moneyNumber(j.amount);return acc},{})).sort((a,b)=>b.revenue-a.revenue);
 const serviceTotal=services.reduce((s,x)=>s+x.revenue,0);
 const exportReport=()=>exportToPDF('Izvještaj poslovanja','Podaci iz trenutne baze',['Radnik','Završeni poslovi','Učinak'],workers.map(w=>[w[0],w[1],`${w[2]}%`]));
 return <><Header eyebrow="ANALITIKA POSLOVANJA" title="Izvještaji" description="Prihod, naplata, produktivnost i prodajni rezultat iz stvarnih podataka."/><div className="report-filter"><div><label>Radnik</label><div className="select-icon report-select"><Users/><Select value={worker} onChange={setWorker}>{workerNames.map(w=><option key={w}>{w}</option>)}</Select></div></div><button className="secondary export-btn" onClick={exportReport}><Download/> Izvezi PDF</button></div><div className="module-metrics four"><Metric icon={WalletCards} label="Ukupno naplaćeno" value={money(revenue)} sub="Prema računima"/><Metric icon={BriefcaseBusiness} label="Završeni poslovi" value={completed.length} sub="Stvarni zapisi" tone="green"/><Metric icon={FileText} label="Prihvaćene ponude" value={`${rate}%`} sub={`${accepted} prihvaćenih`} tone="orange"/><Metric icon={Clock3} label="Otvoreni poslovi" value={filteredJobs.filter(j=>j.status!=='Završeno').length} sub="Trenutno aktivni" tone="purple"/></div><div className="reports-grid"><section className="card"><div className="card-head"><div><h3>Učinak radnika</h3><p>Prema statusu stvarnih poslova</p></div></div><div className="worker-performance">{workers.filter(w=>worker==='Svi radnici'||w[0]===worker).map(x=><div key={x[0]}><span className="avatar">{x[0].split(' ').map(y=>y[0]).join('')}</span><div><strong>{x[0]}</strong><span><i style={{width:`${x[2]}%`}}></i></span></div><b>{x[1]}</b></div>)}{!workers.length&&<Empty/>}</div></section><section className="card report-wide"><div className="card-head"><div><h3>Usluge po ostvarenom prihodu</h3><p>Samo završeni poslovi sa evidentiranim iznosom</p></div></div><table><thead><tr><th>Usluga</th><th>Broj poslova</th><th>Prihod</th><th>Prosječna vrijednost</th><th>Udio</th></tr></thead><tbody>{services.map(x=>{const share=serviceTotal?Math.round(x.revenue/serviceTotal*100):0;return <tr key={x.name}><td><strong>{x.name}</strong></td><td>{x.count}</td><td><strong>{money(x.revenue)}</strong></td><td>{money(x.revenue/x.count)}</td><td><span className="share-bar"><i style={{width:`${share}%`}}></i></span>{share}%</td></tr>})}</tbody></table>{!services.length&&<Empty/>}</section></div></>}
